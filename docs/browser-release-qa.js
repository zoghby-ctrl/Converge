...
async (page) => {
  ...
  let result = await compare('Hide image evidence', true);
  ...
  const restored = await compare('Hide image evidence', false);
  ...
  result = await compare('Add 10 duplicate reports', true);
  ...
  await compare('Add 10 duplicate reports', false);
  let release, started;
  ...
  await page.getByRole('button', {name: 'Switch to Demo Replay', exact: true}).click();
  await loaded;
  release();
